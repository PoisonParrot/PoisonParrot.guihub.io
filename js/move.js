...
function Top(){
    for(var i = 1; i < 5; i++){
        for(var j = i; j <= i + 12; j += 4){
            for(var k = j; k > 4; k -= 4){
                var piece_1 = document.getElementById(k - 4);
                var piece_2 = document.getElementById(k);
...
function Left(){
    for(var i = 1; i <= 13; i += 4){
        for(var j = i; j <= i + 3; j += 1){
            for(var k = j; k > i; k -= 1){
                var piece_1 = document.getElementById(k - 1);
                var piece_2 = document.getElementById(k);
                Change(piece_1,piece_2);
            }
        }
...
function touch_move(){
    ...
    var endY = 0;
    // 获取棋盘对象
    var table = document.getElementById("checkerBoard");
    // 给棋盘绑定'touchstart'事件
